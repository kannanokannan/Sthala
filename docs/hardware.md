...
[sthala-detect] -> Recommended tier: 1
[sthala-detect] -> Suggested profile: --profile accounting-firm
[sthala-detect] -> Suggested model:   Llama-3.1-8B Q4_K_M
```
...
- GPU is optional - Tier 0 CPU-only mode is fully supported
- ECC RAM recommended for Tier 2+ (data integrity on long batch jobs)
